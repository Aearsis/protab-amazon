...
def gen_stock(i):
    ...
    # Normalize all stock to the same yield over gametime, spread the values a bit
    start_off = int((game_start - inception).total_seconds())
    coef = game_sec * 20 / numpy.sum(levels[start_off:start_off + game_sec])
    offset = -10
    return levels * coef + offset
...
def get_stock_value(i, time):
    ...
    return stock[i][int(delta.total_seconds())]
